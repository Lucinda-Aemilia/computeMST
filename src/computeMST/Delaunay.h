...
namespace ayron_delaunay
{
    ...
    class Point
    {
      public:
        ...
        friend Point operator * (const double &s, const Point &b)
        {
          return Point(s*b.x, s*b.y, s*b.z);
        }

        friend Point operator * (const Point &b, const double &s)
        {
          return s*b;
        }
    ...
    class Triangle
    {
      public:
        ...
        friend std::ostream& operator << (std::ostream& os, const Tptr& T)
        {
    ...
    class DelaunayTriangulation
    {
      public:
        ...
        DelaunayTriangulation(double width, double height)
        {
        ...
        DelaunayTriangulation(std::vector<Point>& pointList)
        {
            double width = -1e20, height = -1e20;
            int n = pointList.size();
            for (int i = 0; i < n; i++)
            {
                width = std::max(width, pointList[i].x);
                height = std::max(height, pointList[i].y);
            }
        ...
        std::vector<Point> getPointList() { return points; }

        std::vector<std::pair<int, int>> getVoronoiEdge()
        {
            std::vector<std::pair<int, int>> voronoi;
            std::pair<int, int> e;

            for (auto t : triangles)
            {
                int p[3] = {t->v[0], t->v[1], t->v[2]};
                std::sort(p, p+3);

                e = std::make_pair(p[0], p[1]);
                voronoi.push_back(e);
                e = std::make_pair(p[0], p[2]);
                voronoi.push_back(e);
                e = std::make_pair(p[1], p[2]);
                voronoi.push_back(e);
            }
            std::sort(voronoi.begin(), voronoi.end());
            voronoi.erase(unique(voronoi.begin(), voronoi.end()),voronoi.end());
            return voronoi;
        }
